...
{
#t/* Init USB Ip. */
#thpcd_USB_DRD_FS.pData = pdev;
#t/* Link the driver to the stack. */
#tpdev->pData = &hpcd_USB_DRD_FS;
...
[#include mxTmpFolder+"/usb_drd_fs_HalInit.tmp"]
  ...
  [/#if]
  [#if useBCD]
  HAL_PCD_RegisterBcdCallback(&hpcd_USB_DRD_FS, PCDEx_BCDCallback);
  [/#if]
...
[#if BspUsed=true]
  if (GPIO_Pin == ${BCD_PIN})
  {
    if (HAL_GPIO_ReadPin(${BCD_PORT}, ${BCD_PIN}) == GPIO_PIN_SET)
    {
      ...
      HAL_Delay(450);
      /*Start BCD Detect*/
      HAL_PCDEx_ActivateBCD (&hpcd_USB_DRD_FS);
      HAL_PCDEx_BCD_VBUSDetect(&hpcd_USB_DRD_FS);
    }
    else
    {
