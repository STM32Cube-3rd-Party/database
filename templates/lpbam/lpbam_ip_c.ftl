               ...
               [#list initService.clock?split(';') as clock]
                  ...
                  #t#t#t${clock?replace("ENABLE","DISABLE")?trim}(); 
                            ...
                            [#else]
                             ...
                             #t#t*#t#t${clock?replace("ENABLE","DISABLE")?trim}[#if !clock?contains('(')]()[/#if]; 
                             #t#t**/
                        [/#if]
                    [#else]
    #t#t${clock?replace("ENABLE","DISABLE")?trim}[#if !clock?contains('(')]()[/#if];
                    [/#if]
               [/#list]
            [/#if]
         [#else]
...
/**
#t* @brief ${instName} initialization.
#t* @param None
#t* @retval None
#t*/
        [#if halMode!=instanceData.realIpName&&!instanceData.ipName?contains("TIM")&&!instanceData.ipName?contains("CEC")]#nstatic void MX_${instName}_Init(void)[#else]static void MX_${instName}_Init(void)[/#if]
{
...
#n
[#-- lpbam ***set instance and register callback Start --]
[#if instanceData.usedDriver?? && instanceData.usedDriver=="HAL"]
#t/* Set ${instName} instance */
[#assign hnadlerName = ""]
[#if instanceData.instancehandler??]
[#assign hnadlerNameD = instanceData.instancehandler]
[#if hnadlerNameD?? && hnadlerNameD!="" ]
[#assign hnadlerName = hnadlerNameD?split(" ")[1]]
[/#if]
[/#if]
[#-- hdac1.Instance = DAC1; --]
...
[/#if]
        [#list ipvar.initCallBacks.entrySet() as entry]#n
          ...
          #t/* Register ${mode} msp callbacks */
[#if mode == "OPAMP"]
          #tif (HAL_${mode}_RegisterCallback(&${hnadlerName}, HAL_${mode}_MSP_INIT_CB_ID, MX_${instName}_MspInit) != HAL_OK)
[#else]
          #tif (HAL_${mode}_RegisterCallback(&${hnadlerName}, HAL_${mode}_MSPINIT_CB_ID, MX_${instName}_MspInit) != HAL_OK)
[/#if]
          #t{
            #t#tError_Handler();
          #t}
          ...
          #t/* Register ${mode} msp callbacks */
[#if mode == "OPAMP"]
          #tif (HAL_${mode}_RegisterCallback(&${hnadlerName}, HAL_${mode}_MSP_DEINIT_CB_ID, MX_${instName}_MspDeInit) != HAL_OK)
[#else]
          #tif (HAL_${mode}_RegisterCallback(&${hnadlerName}, HAL_${mode}_MSPDEINIT_CB_ID, MX_${instName}_MspDeInit) != HAL_OK)
[/#if]
          #t{
            #t#tError_Handler();
          #t}
    ...
    [#if instName?starts_with("DAC")] [#-- Call HAL_DAC_DeInit() --]
    #tif (HAL_DAC_DeInit(&${hnadlerName}) != HAL_OK)
              #t{
                #t#tError_Handler();
              #t}
    [/#if]
[/#if] [#-- if instanceData.usedDriver?? && instanceData.usedDriver!="HAL" --]
[#-- lpbam ***set instance and register callback END --]
...
[#compress]
[#list IP.configModelList as instanceData]
[#if instanceData.isMWUsed=="false" && instanceData.isBusDriverUSed=="false" && !instanceData.instanceName?starts_with("LPDMA")  && !instanceData.instanceName?starts_with("TAMP") && !instanceData.instanceName?starts_with("PWR")]
     [#assign instName = instanceData.instanceName]
        [#assign halMode= instanceData.halMode]
[#if instName!="VREFBUF" && instName!="GPIO"]
#n
/**
#t* @brief  ${instName} de-initialization.
#t* @param  None
#t* @retval None
#t*/
static void MX_${instName}_DeInit(void)
{
#t/* USER CODE BEGIN ${instName}_DeInit 0 */
#n#t/* USER CODE END ${instName}_DeInit 0 */
#n
 [#if ipvar.initCallBacks??]
...
[#if mode == "OPAMP"]
  #tif (HAL_${mode}_UnRegisterCallback(&${hnadlerName}, HAL_${mode}_MSP_INIT_CB_ID) != HAL_OK)
[#else]
  #tif (HAL_${mode}_UnRegisterCallback(&${hnadlerName}, HAL_${mode}_MSPINIT_CB_ID) != HAL_OK)
[/#if]
  #t{
  #t#tError_Handler();
  #t}
...
[#else]
  #tif (HAL_${mode}_UnRegisterCallback(&${hnadlerName}, HAL_${mode}_MSPDEINIT_CB_ID) != HAL_OK)
[/#if]
  #t{
  #t#tError_Handler();
  #t}
    ...
    [/#if] [#-- if ipvar.initCallBacks?? --]
#n
#t/* USER CODE BEGIN ${instName}_DeInit 1 */
#n#t/* USER CODE END ${instName}_DeInit 1 */ 
}
[/#if]
[/#if] 
[/#list]
...
[#--if name !="TIM"--]
/**
#t* @brief ${instName} MSP initialization.
#t* @retval None
#t*/
static void MX_${instName}_MspInit(${mode}_HandleTypeDef* ${mode?lower_case}Handle)
{#n
[#-- else --]
...
[#assign ipHandler = "h" + mode?lower_case]
/**
#t* @brief ${instName} MSP de-initialization.
#t* @retval None
#t*/
[#if name !="TIM"]
static void MX_${instName}_MspDeInit(${mode}_HandleTypeDef* ${mode?lower_case}Handle)
{#n
[#else]
#nstatic void MX_${instName}_MspDeInit(${name}_HandleTypeDef* ${mode?lower_case}Handle)
{#n
[/#if]
#n
[#if mspIsEmpty=="no"]
