...
/**
  * @brief ADC handle
  */
extern ADC_HandleTypeDef h${IpInstance?lower_case};
/* USER CODE BEGIN EV */
...
void SYS_InitMeasurement(void)
{
  ...
  h${IpInstance?lower_case}.Instance = ${IpInstance};
...
static uint32_t ADC_ReadChannels(uint32_t channel)
{
  ...
  /* Start Calibration */
  if (HAL_ADCEx_Calibration_Start(&h${IpInstance?lower_case}) != HAL_OK)
  {
    Error_Handler();
  }
  ...
  if (HAL_ADC_ConfigChannel(&h${IpInstance?lower_case}, &sConfig) != HAL_OK)
  {
    Error_Handler();
  }

  if (HAL_ADC_Start(&h${IpInstance?lower_case}) != HAL_OK)
  {
    /* Start Error */
    Error_Handler();
  }
  /** Wait for end of conversion */
  HAL_ADC_PollForConversion(&h${IpInstance?lower_case}, HAL_MAX_DELAY);

  /** Wait for end of conversion */
  HAL_ADC_Stop(&h${IpInstance?lower_case});   /* it calls also ADC_Disable() */

  ADCxConvertedValues = HAL_ADC_GetValue(&h${IpInstance?lower_case});

  HAL_ADC_DeInit(&h${IpInstance?lower_case});
