...
#define HAL_MODULE_ENABLED  
  ...
  [#assign allModules = ["ADC","COMP","CORDIC","CRC","AES","DAC","FDCAN","FMAC","HRTIM","IRDA","IWDG","I2C", "I2S","LPTIM","NAND","NOR","OPAMP","PCD","QUADSPI","RNG","RTC","SAI","SMARTCARD","SMBUS","SPI","SRAM","TIM","UART","USART","WWDG"]]
...
#define LSI_VALUE  ([#if lsi_value??]${lsi_value}[#else]32000[/#if]UL)     /*!< LSI Typical Value in Hz*/    
#endif /* LSI_VALUE */
/**
...
#define LSE_VALUE  ([#if lse_value??]${lse_value}[#else]32768[/#if]UL)    /*!< Value of the External Low Speed oscillator in Hz */
#endif /* LSE_VALUE */     
...
#define EXTERNAL_CLOCK_VALUE    ([#if external_clock_value??]${external_clock_value}[#else]48000[/#if]UL) /*!< Value of the External oscillator in Hz*/
...
#define  PREFETCH_ENABLE              [#if PREFETCH_ENABLE??]${PREFETCH_ENABLE}U[#else]0U[/#if]
#define  INSTRUCTION_CACHE_ENABLE     [#if INSTRUCTION_CACHE_ENABLE??]${INSTRUCTION_CACHE_ENABLE}U[#else]1U[/#if]
