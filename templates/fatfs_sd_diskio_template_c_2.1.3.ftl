...
[#if use_dma = 1]
 [#if use_rtos = 1]
/* Note: code generation based on sd_diskio_dma_rtos_template_bspv1.c v2.1.3 as FreeRTOS is enabled. */
 [#else]
/* Note: code generation based on sd_diskio_dma_template_bspv1.c v2.1.3 as "Use dma template" is enabled. */
 [/#if]
[#else]
/* Note: code generation based on sd_diskio_template_bspv1.c v2.1.3 as "Use dma template" is disabled. */
[/#if]
...
#if defined (ENABLE_SD_DMA_CACHE_MAINTENANCE)[#-- Code only for Cortex M7!!! --]
...
[#--Modified to support v2.1.0 templates --]
[#if use_dma = 1]
...
static DSTATUS SD_CheckStatus(BYTE lun)
{
 ...
 [/#if]
[#else]                 [#--Part from sd_diskio_template.c --]
static DSTATUS SD_CheckStatus(BYTE lun)
{
  Stat = STA_NOINIT;
...
{
[#-- Stat = STA_NOINIT;  -->  Not in sd_diskio_dma_template.c but in the two others --]
[#if use_dma = 1]
 [#if use_rtos = 1]
Stat = STA_NOINIT; 
 [/#if]
...
#endif
          memcpy((void *)buff, scratch, BLOCKSIZE);
          buff += BLOCKSIZE;
        }
        else
        {
...
/**
  ...
  * @retval DRESULT: Operation result
  */
[#--Part common to the 3 templates --] 
#if _USE_IOCTL == 1
...
[#if use_dma = 1]    [#--Part not in sd_diskio_template.c, but only when using dma --]
...
[#--Added to support v2.1.0 template --]
...
[/#if]  [#--End of the part not in sd_diskio_template.c, but only when using dma --]
