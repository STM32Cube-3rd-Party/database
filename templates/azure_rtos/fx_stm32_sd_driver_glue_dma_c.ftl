...
extern SD_HandleTypeDef hsd${sd_instance+1};
#if (FX_STM32_SD_INIT == 1)
extern void MX_SDMMC${sd_instance+1}_SD_Init(void);
#endif
...
#if (FX_STM32_SD_INIT == 1)
	MX_SDMMC${sd_instance+1}_SD_Init();
#endif
...
INT fx_stm32_sd_deinit(UINT instance)
{
  ...
  if(HAL_SD_DeInit(&hsd${sd_instance+1}) != HAL_OK)
  {
    ret = 1;
  }
...
INT fx_stm32_sd_get_status(UINT instance)
{
  ...
  if(HAL_SD_GetCardState(&hsd${sd_instance+1}) != HAL_SD_CARD_TRANSFER)
  {
    ret = 1;
  }
...
void HAL_SD_TxCpltCallback(SD_HandleTypeDef *hsd)
{
/* USER CODE BEGIN HAL_SD_TxCpltCallback */

  /* USER CODE END HAL_SD_TxCpltCallback */
}
[/#if]  
