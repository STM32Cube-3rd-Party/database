...
[#list 0..(total_channels_nb-1) as i]
    { CHANNEL_${i}_SRC, CHANNEL_${i}_IO_MSK, CHANNEL_${i}_GRP_MSK }[#if i!=total_channels_nb],[/#if]
[/#list]
};
[#-- ************************************************************************************************** --]
...
[#-- list of channels  1 per key, 3 per linear/rot, if n lin/rot activated then 3*n first GPIO affected --]
[#list 0..(total_channels_nb-1) as i]
    { CHANNEL_${i}_DEST }[#if i!=total_channels_nb],[/#if]
[/#list]
[#-- ************************************************************************************************** --]
};
                ...
                [#list 0..wordsArraySize as i]
                    ...
                    [#if wordsArrayItem?contains("TSLPRM")]
                        ...
                        [#list 0..wordsArrayItemNextNbr as j]
                            ...
                            [#lt]   {&MyChannels_Src[${counter}], &MyChannels_Dest[${counter}], MyChannels_Data, BANK_${bankNbInt}_NBCHANNELS, BANK_${bankNbInt}_MSK_CHANNELS, BANK_${bankNbInt}_MSK_GROUPS},
...
[/#if]
    ...
    [/#if]
    [#if total_linrots_b_nb != 0]
        [#lt]CONST TSL_LinRotB_T MyLinRotsB[TSLPRM_TOTAL_LINROTS_B] =
        [#lt]{
        [#list SWIPdatas as SWIP]
            [#if SWIP.defines??]
                [#list SWIP.defines as define]
