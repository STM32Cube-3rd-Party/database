...
[#list SWIPdatas as SWIP]  
	[#if SWIP.defines??]
		[#list SWIP.defines as definition]
			[#if definition.name="MASTER_CPU_ID"]
			[#lt]#define MASTER_CPU_ID    ${definition.value}
                        [/#if]
                        [#if definition.name="REMOTE_CPU_ID"]
                        [#lt]#define REMOTE_CPU_ID    ${definition.value}
			[/#if]
                        [#if definition.name="IPCC_CPU_A7"]
                        [#lt]#define IPCC_CPU_A7      ${definition.value}
			[/#if]
