...
uint8_t * USBD_${CLASS_FS}_DeviceDescriptor(USBD_SpeedTypeDef speed, uint16_t *length);
uint8_t * USBD_${CLASS_FS}_LangIDStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length);
uint8_t * USBD_${CLASS_FS}_ManufacturerStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length);
uint8_t * USBD_${CLASS_FS}_ProductStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length);
uint8_t * USBD_${CLASS_FS}_SerialStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length);
uint8_t * USBD_${CLASS_FS}_ConfigStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length);
uint8_t * USBD_${CLASS_FS}_InterfaceStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length);

[#-- Ticket 66338 
#ifdef USBD_SUPPORT_USER_STRING_DESC
uint8_t * USBD_${CLASS_FS}_USRStringDesc(USBD_SpeedTypeDef speed, uint8_t idx, uint16_t *length);
...
uint8_t * USBD_${CLASS_FS}_USR_BOSDescriptor(USBD_SpeedTypeDef speed, uint16_t *length);
...
USBD_DescriptorsTypeDef ${CLASS_FS}_Desc =
{
  USBD_${CLASS_FS}_DeviceDescriptor, 
  USBD_${CLASS_FS}_LangIDStrDescriptor, 
  USBD_${CLASS_FS}_ManufacturerStrDescriptor, 
  USBD_${CLASS_FS}_ProductStrDescriptor, 
  USBD_${CLASS_FS}_SerialStrDescriptor, 
  USBD_${CLASS_FS}_ConfigStrDescriptor, 
  USBD_${CLASS_FS}_InterfaceStrDescriptor
[#if family?contains("STM32F7") || family?contains("STM32F4") || family?contains("STM32L4") || family?contains("STM32WB") || family?contains("STM32G4") || family?contains("STM32L5")]
[#if useLPM=true]
#if (USBD_LPM_ENABLED == 1)
, USBD_${CLASS_FS}_USR_BOSDescriptor
#endif /* (USBD_LPM_ENABLED == 1) */
[/#if]
[/#if]
};
...
__ALIGN_BEGIN uint8_t USBD_${CLASS_FS}_DeviceDesc[USB_LEN_DEV_DESC] __ALIGN_END =
{
  0x12,                       /*bLength */
...
{
  UNUSED(speed);
  *length = sizeof(USBD_${CLASS_FS}_DeviceDesc);
  return USBD_${CLASS_FS}_DeviceDesc;
}
...
uint8_t * USBD_${CLASS_FS}_SerialStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length)
{
  ...
  *length = USB_SIZ_STRING_SERIAL;
  ...
  /* USER CODE BEGIN USBD_${CLASS_FS}_SerialStrDescriptor */
  
  /* USER CODE END USBD_${CLASS_FS}_SerialStrDescriptor */
