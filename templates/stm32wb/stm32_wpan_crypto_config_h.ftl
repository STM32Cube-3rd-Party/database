 ...
 *
 */
#if  defined(TFM_HW_ACCEL_ENABLE)
...
#if defined(TFM_HW_ACCEL_ENABLE)
/**
 * \def HW_CRYPTO_DPA_AES
 *
 * Allow DPA resistance for AES modes by using secure crypto processor (SAES)
 *
 * When this option is enabled, all AES modes become DPA-protected.
 *
 * \note : All AES modes mean AES-ECB/CBC but also modes with an implementation
 *         on top of them. This includes CTR, XTS, CFB, OFB.
 *
 * Tradeoff: Using DPA resistance degrades the performance.
 *           Comment this option if your system can run cryptographic services
 *           without DPA resistance for the benefit of the highest performance
 *           in the TFM run-time application.
 *
 * Requires: MBEDTLS_AES_ALT
 */
#define HW_CRYPTO_DPA_AES

/**
 * \def HW_CRYPTO_DPA_GCM
 *
 * Allow DPA resistance for GCM by using secure crypto processor (SAES)
 *
 * When this option is enabled, GCM becomes DPA-protected.
 *
 * \warning  Disabling this switch negates any security provided by the TFM
 *           and set up for Secure Storage service.
 *
 * Requires: MBEDTLS_GCM_ALT.
 */
#define HW_CRYPTO_DPA_GCM


/**
 * \def HW_CRYPTO_DPA_CCM
 *
 * Allow DPA resistance for CCM by using secure crypto processor (SAES)
 *
 * \warning  AES-CCM always remains on AES (not implemented).
 */
/* #define HW_CRYPTO_DPA_CCM */

/**
 * \def USE_HUK
 *
 * Allow using of Hardware Unique Key in crypto operation driven
 * from TFM and crypto MWs.
 *
 * \note : Hardware Unique Key is used for Secure Storage service
 *
 * Requires: HW_CRYPTO_DPA_GCM
 */
#define USE_HUK

#if defined(USE_HUK) && !defined(HW_CRYPTO_DPA_GCM)
#error "USE_HUK defined, but not all prerequisites (HUK is used with a DPA-protected GCM)"
#endif

#if defined(MBEDTLS_CCM_ALT) && defined(HW_CRYPTO_DPA_CCM)
#error "MBEDTLS_CCM_ALT is not supported with DPA protection"
#endif

#endif /* TFM_HW_ACCEL_ENABLE */
