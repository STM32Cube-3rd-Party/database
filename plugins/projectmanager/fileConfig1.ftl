...
[#-- <ProjectPath>${ProjectPath}</ProjectPath>  --][#-- added to give project path --]
    <ProjectName>${projectName}[#if TrustZone=="1"][#if prjSecure=="1"]_S[#else]_NS[/#if][/#if][#if MultiProject=="1"]_${cpucore?replace("ARM Cortex-", "C")}[/#if]</ProjectName> [#-- modified to give only the project name without path --]
